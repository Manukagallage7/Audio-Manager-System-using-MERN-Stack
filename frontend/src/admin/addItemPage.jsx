...
export default function AddItemPage() {
    ...
    async function handleAddItem() {
        ...
        if(token) {
            try{
                const result = await axios.post("http://localhost:5000/api/product", {
                    key: ItemKey,
                    name: ItemName,
                    price: ItemPrice,
    ...
    return (
        <div className="flex flex-col w-full h-full items-center gap-5 pt-10">
            ...
            <div className="flex flex-col w-[400px] backdrop-blur-2xl border-0 gap-2 p-5 rounded-sm bg-yellow-300">
                ...
                <button onClick={handleAddItem} className="w-full h-[40px] bg-green-400 rounded-sm hover:bg-green-500">Add Item</button>
                <button onClick={()=> {navigate("/adminPage/items")}} className="w-full h-[40px] bg-green-400 rounded-sm hover:bg-green-500">Cancel</button>
            </div>
        </div>
    )
}